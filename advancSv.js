...
<p>До появления рун в Svelte 5, хранилища (stores) были идиоматичным способом обработки реактивного состояния вне компонентов. Сейчас это уже не так, но вы все равно будете сталкиваться с хранилищами при использовании Svelte (включая SvelteKit, пока что), поэтому стоит знать, как их использовать. Потому что, writable и $-префикс из ранних версий Svelte всё ещё работают!<br><strong> Забудьте про writable и $-префикс, если вам просто нужно передавать данные между файлами!!!  </strong> </p>
...
<div>
    <ul>
      ...
      <li>Название файла: <mark style="color: darkblue; font-weight:800; padding: 4px 12px;">Всегда .svelte.js,</mark> иначе руны $state не заработают.</li>
      <li>Защита: Все основные переменные — через #.<mark style="color: darkblue; font-weight:800; padding: 4px 12px;"> Наружу только get.</mark></li>
...
<p>Использовать set, необходимо, чтоб изменить данные и сразу что-то сделать (проверить, сохранить, отправить лог).</p>
...
<p>Мы можем вызвать этот сеттер в другом компоненте и передать ему значение, как <mark style="background-color:orange;">
DataAppStore.addCheckNow = 20 </mark> - выполнится проверка, что 20 > 0 и значение startNow увеличится на 20 единиц. <mark style="background-color:orange;">В JavaScript сеттер работает не как обычная функция, а через знак равно.</mark></p>
  </li>
  <li>
...
<div style="line-height:1.75rem; border: 12px groove #fff; background-color: #DAA;">
...
<details  style=" background-color: rgba(190, 114, 190, 0.91); padding: 1rem 2rem;">
...
section.insertAdjacentHTML('beforebegin', `<a href="#link${taskCurrent}">${taskCurrent}.  ${nameTask}</a><br>`);
section.insertAdjacentHTML('beforeend', `<hr><hr><h3 id="link${taskCurrent}" class="title"> <span id="${nameTask}"></span>   task  ${taskCurrent}. ${nameTask} </h3><hr><hr><article id="${nameTask}" class="article">
  <a href="https://svelte.dev/tutorial/svelte/snippets-and-render-tags">tutorial</a>

  <p><strong> Сниппет — это «функция», которая возвращает не число или строку, а кусок HTML-разметки. </strong></p>
  <p>Фрагменты кода позволяют повторно использовать контент внутри компонента, не вынося его в отдельный файл.</p>
<p>В Svelte 5 на смену классическим слотам (<slot />) пришли Сниппеты ({#snippet}).</p>
 
<h5>🛠 Как это работает</h5>
<ol>
<p>Для работы со сниппетами используются два ключевых слова:</p>

<li>{#snippet name(params)} — здесь мы создаем (объявляем) шаблон.</li>
<li>{@render name(params)} — здесь мы выводим его на экран.</li>
</ol>

<details>
  <summary>Пример: Карточка раунда в игре</summary>
  <p>Допустим, нужно выводить статус раунда. Вместо того чтобы дублировать верстку для каждого раунда, мы создадим сниппет.</p>
  <pre>
    <code>
      &#60;script>
        let rounds = $state([
          { id: 1, status: 'win', score: 100 },
          { id: 2, status: 'lose', score: 0 },
          { id: 3, status: 'next', score: 0 }
        ]);
      &#60;/script>

      {#snippet roundCard(round)}
        &#60;div class="card {round.status}">
          &#60;h4>Раунд №{round.id}&#60;/h4>
          &#60;p>Статус: {round.status.toUpperCase()}&#60;/p>
          {#if round.status === 'win'}
            &#60;span>Очки: {round.score}&#60;/span>
          {/if}
        &#60;/div>
      {/snippet}
      ...
      &#60;style>
        ...
        .win { border-color: green; background: #e6ffed; }
        .lose { border-color: red; background: #fff5f5; }
        .next { border-color: gray; opacity: 0.5; }
      &#60;/style>
    </code>
  </pre> 
</details>
<h5>Достоинства {#snippet name(params)}</h5>
<p><mark>1. Параметры (Аргументы)</mark></p>
<p>В отличие от старых слотов, в сниппет можно передавать любые переменные, как в обычную функцию. Это делает их невероятно гибкими.</p>

<p><mark>2. Передача сниппетов в другие компоненты</mark></p>
<p>В Svelte 5 сниппеты — это просто свойства (props). Ты можешь передать сниппет в дочерний компонент, чтобы тот отрисовал его внутри себя.</p>

Например: У тебя есть компонент Modal.svelte. Ты можешь передать в него сниппет header и сниппет footer, и модальное окно само решит, куда их вставить.

<p><mark>3. Производительность</mark></p>
<p>Сниппеты гораздо «легче» полноценных компонентов. Если тебе нужно повторить кусок верстки 100 раз внутри одного файла, лучше использовать сниппет, а не создавать 100 экземпляров отдельного .svelte файла.</p>
</article>`);
