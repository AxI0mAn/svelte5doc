...
<h4>📝 Вывод: В Svelte 5 вы просто обновляете переменную и DOM обновляется автоматически.</h4>
...
<h5>Механизм Скрытия/Показа:</h5>
<p><mark>Синтаксис:</mark> Используется специальный синтаксис шаблона Svelte:</p>
<pre>
<code>
...
<pre>
<code> 
numbers['three'] = 3; // <-- <strong>(Мутация) — Рекомендуемый способ ✅</strong>
numbers = { ...numbers,  'three': 3 }; // <-- <strong>Создаем НОВЫЙ объект и добавляем свойство</strong>
</code>
</pre> 
<p>В Svelte 5 оба варианта технически сработают, если объект numbers был объявлен через руну $state().<br><strong> Однако между ними есть принципиальная разница в том, как Svelte обрабатывает изменения.</strong></p>

<pre>
<h5>Вариант: numbers['three'] = 3; <mark>(Мутация) — Рекомендуемый способ ✅</mark></h5>
Благодаря тому, что $state() использует Proxy, 
Svelte перехватывает добавление нового свойства.

Почему это лучше:

<mark style="background-color:green;">Производительность:</mark> Svelte не нужно создавать копию всего объекта. 
Он просто добавляет ключ и уведомляет систему реактивности.

<mark style="background-color:green;">Точечные обновления (Fine-grained): </mark>
Если другие части кода используют старые свойства объекта (например, numbers.one), 
они не будут пересчитываться или перерисовываться. 
Обновится только тот кусочек интерфейса, который ждал появления three.

<mark style="background-color:green;">Чистота кода:</mark> Код выглядит как обычный JavaScript.
</pre>

<hr>
<pre>
<h5>Вариант: numbers = { <mark style="background-color:pink;">...numbers,</mark> 'three': 3 }; <mark style="background-color:pink;">(Пересоздание) — Старый стиль ⚠️</mark></h5>
Этот способ пришел из Svelte 3/4 и React, 
где иммутабельность (неизменяемость) была обязательной.

Что происходит:
Вы создаете абсолютно новый объект в памяти.
Вы заменяете ссылку на объект.

<mark style="background-color:pink;">Проблема:</mark> Svelte увидит, что весь объект numbers изменился. 
Это может привести к лишним проверкам во всех компонентах, 
которые используют любые свойства этого объекта, даже если они не менялись.
</pre>
...
<mark style="background-color:pink;">  ИЛИ УСТАРЕВШИЙ ВАРИАНТ, КОТОРЫЙ приводит к излишним вычислениям):</mark>:
...
&lt;script>
	...
	function addProperty() {
		...
		numbers = { <mark style="background-color:pink;">...numbers,</mark> 'three': 3 };
	}
&lt;/script>
...
&lt;script>
	...
	function changeProperty() {
        ...
        //  <mark style="background-color:pink;">но приводит к излишним вычислениям):</mark>
        // numbers = { <mark style="background-color:pink;">...numbers,</mark> 'two': 22 };
	}
&lt;/script>
...
<h5><strong>В Svelte 5 идеология проста: старайся делать всё через $state и $derived, а если это невозможно — используй $effect.</strong></h5>
...
<h5>🛑 Когда НЕЛЬЗЯ использовать $effect</h5>
<p>Главная ошибка новичков — использовать эффект для синхронизации данных.</p>
<ul>
<li><mark>Для вычислений: </mark>Если тебе нужно значение, которое зависит от другого (например, fullName = firstName + lastName), используй $derived.</li>

<li><mark>Для изменения стейта на основе стейта: </mark>Если при изменении одной переменной ты хочешь вручную поменять другую (например, if (points > 100) level += 1), это плохой путь. Логику изменения лучше вынести в функцию (action), которая вызывается при клике или событии.</li>
</ul>

<h5>✅ Когда НУЖНО использовать $effect</h5>
<p><mark>Эффект предназначен для «побочных эффектов» (side effects) — это действия, которые выходят за рамки чистого Svelte и взаимодействуют с внешним миром.</mark></p>
<ol>
<li><mark>Работа с DOM-библиотеками</mark><br>
Если тебе нужно инициализировать стороннюю библиотеку (графики Chart.js, карты Leaflet, редакторы текста), которая должна «присосаться» к HTML-элементу после того, как он появился на странице.

</li><li><mark>Взаимодействие с API браузера</mark><br>
Всё, что Svelte не контролирует напрямую:<br>

Установка фокуса на элемент (input.focus()).<br>

Работа с canvas (рисование).<br>

События window или document (например, отслеживание движения мыши или прокрутки).

</li><li><mark>Таймеры и интервалы</mark><br>
Если тебе нужно запустить setInterval или setTimeout, которые зависят от состояния игры.

</li><li><mark>Синхронизация с внешними хранилищами</mark><br>
Например, если ты хочешь автоматически сохранять игру в localStorage всякий раз, когда меняется объект со счетом.</li>
</ol>

<details>
<summary>Пример кода: в игре нужно проигрывать звук, когда меняется счет, и сохранять прогресс.</summary>
<pre>
<code>
    &#60;script>
      let score = $state(0);
      ...
      // Это правильное использование $effect
      $effect(() => {
        // Этот код сработает ПРИ КАЖДОМ изменении score
        console.log("Счет изменился, сохраняем...");
        localStorage.setItem('game_score', score);

        if (score > 0 && soundEnabled) {
        ...
        return () => {
          console.log("Эффект перед следующим запуском или уничтожением компонента");
        };
      });
    &#60;/script>

    &#60;button onclick={() => score += 10}>Добавить очки&#60;/button>
</code>
</pre>
</details>

<h5>💡 Особенности, которые важно знать</h5>
<ol>
<li><mark>Запуск после рендеринга:</mark> $effect всегда запускается после того, как изменения в DOM уже применились. Это гарантирует, что ты работаешь с актуальными HTML-элементами.</li>

<li><mark>Автоматические зависимости:</mark> Тебе не нужно указывать список зависимостей (как в useEffect в React). Svelte сам «видит», какие реактивные переменные ты использовал внутри кода эффекта, и переподписывается на них.</li>

<li><mark>Очистка (Cleanup):</mark> Если ты возвращаешь функцию из $effect, Svelte выполнит её прямо перед тем, как запустить эффект снова, или когда компонент будет удален. Это критично для удаления addEventListener или остановки таймеров.</li>
</ol>
</article>`);
...
<div style='border: 8px groove white; background-color: rgba(255,255,255, .3); line-height: 2rem; font-size: 1.8rem;'>
<h5 style="margin-bottom:2rem;">💡 Профессиональный совет</h5>

<p>Если вы часто пробрасываете переменные в стили, помните:</p>
<ul>
<li>Вы можете использовать SCSS для вложенности, миксинов и прочих радостей, но для связи с JavaScript используйте именно CSS-переменную.</li>

<li>$variable — используйте для констант, которые никогда не меняются (например, фирменный цвет бренда, сетка сайта).</li>

<li>--variable — используйте для всего, что зависит от логики приложения (цвета игрока, уровень прогресса, координаты курсора, состояние подсказок).</li>
</ul>
</div>
</article>`);
...
# App.svelte
    ...
    &lt;script>
      ...
      &lt;div class="container">
        ...
        {#if showMenu}
          &lt;div
            <strong>role</strong>="presentation" // **пояснение**
          ...
          // **пояснение**  -  атрибут <strong>role</strong> является стандартным атрибутом HTML5. 
