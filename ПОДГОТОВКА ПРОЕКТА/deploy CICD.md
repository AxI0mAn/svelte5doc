...
npm run prod
Эта команда удалит старую папку build и соберет новую с VITE_BASE_PATH для локального тестирования статического сайта (чтобы убедиться, что он работает перед отправкой).
